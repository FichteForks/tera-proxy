...
class Module {
  constructor(dispatch, name, options) {
    ...
    this.settingsFile = (this.settingsVersion === null) ? '' : path.join(options.rootFolder, options.settingsFile || 'module_settings.json');
    this.settingsMigrator = (this.settingsVersion === null) ? '' : path.join(options.rootFolder, options.settingsMigrator || 'module_settings_migrator.js');
    this._command = null;
    this._game = null;
  ...
  loadSettings() {
    ...
    try {
      ...
      console.error(`ERROR: Invalid settings format for module ${this.name}!\nERROR: This means that you broke it when manually editing it. The full error message is:\nERROR: ${e}`);
      process.exit(1);
    }
